...
In the example above, the `println!("Middle");` block will be inserted into the `dynamic` anchor within `main.rs`.
...
```rust hellorust tag='exec' filename='main.rs' pre=|||fn main() {||| post=|||}||| cmd=|||rustc main.rs && ./main|||
println!("Hello, Rust!");
```
